...
'''
Exporting Frozen Graph script for license plate recognition with LPRNet

This script takes a .ckpt file and generates the frozen graph.

Input:
  - path_to_config: path to the configuration file
  - output_dir: folder path in wich the frozen graph will be saved
  - checkpoint: path to the checkpoint (.ckpt) file to convert

Output: two files are generated in output_dir
  - graph.pb.frozen: our frozen graph (you should rename it to "graph.pb")
  - graph.pbtxt: frozen graph (not binary)


Example Command:
  python export_frozen_graph.py config/config_file.py --output_dir frozen_graph/ --checkpoint model/model.ckpt-250000.ckpt

'''


# command:  python export_frozen_graph.py config/config_synthetic_64_quantized.py --output_dir frozen_graph/ --checkpoint pretrained_model/LPRNet_synth_no_tile_64_quantized/model.ckpt-250000.ckpt
#
